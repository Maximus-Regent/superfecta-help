...
def generate_report(all_results: dict, summary: pd.DataFrame, races: list):
    ...
    lines = [
        ...
        "",
        "- Valid evidence scope: `valid_evidence_scope=legacy_broad_backtest_negative_baseline_context_only`.",
        ...
        "- If quoting `CURRENT_EVIDENCE_SUMMARY.*` from this legacy backtest context after scorecard/rules/signals/settlement-ledger byte changes, follow `current_evidence_summary.json.rebuild_validation_contract`: `python3 paper_trade_settlement_audit.py` -> `python3 current_evidence_summary.py` -> `python3 validate_current_evidence_summary.py`; green rebuild checks are provenance metadata only, not settled ROI, promotion readiness, live profitability, bankroll guidance, or real-money evidence.",
