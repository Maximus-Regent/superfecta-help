#!/usr/bin/env python3
"""
Validation for SELECTOR_EXPERIMENT.md current-evidence boundaries.

Purpose:
- keep the selector-scoring experiment framed as historical selector research
- prevent the sqrt|strict adoption wording from being read as a live rule change
...
REPORT = BASE / "SELECTOR_EXPERIMENT.md"
GENERATOR = BASE / "experiment_selector_variants.py"
CURRENT_EVIDENCE_JSON = BASE / "current_evidence_summary.json"
OUT_DIR = BASE / "out" / "status_validation" / "selector_experiment_caution"
OUT_MD = OUT_DIR / "selector_experiment_caution_validation.md"
OUT_JSON = OUT_DIR / "selector_experiment_caution_validation.json"
REBUILD_COMMAND = "python3 validate_selector_experiment_caution.py"
VALID_EVIDENCE_SCOPE = "selector_scoring_replay_diagnostic_only"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate SELECTOR_EXPERIMENT.md caution wording")
...
def current_bridge_cli_contract_checks(current_evidence_json: Path = CURRENT_EVIDENCE_JSON) -> list[dict[str, Any]]:
    ...
    with TemporaryDirectory(prefix="selector_experiment_current_evidence_") as tmp_dir:
        ...
        missing_contract_out_dir = tmp_root / "missing" / "selector_experiment_caution"
        ...
        checks.append(require(
            ...
            "validate_selector_experiment_caution.py rejects a current-evidence sidecar with no rebuild_validation_contract before creating nested output directories or partial selector-experiment validation artifacts",
        ))

        weakened_contract_out_dir = tmp_root / "weakened" / "selector_experiment_caution"
        ...
        checks.append(require(
            ...
            "validate_selector_experiment_caution.py rejects a current-evidence rebuild contract that no longer marks the rebuild order as provenance metadata only before creating nested output directories or partial selector-experiment validation artifacts",
        ))
...
def main(argv: list[str] | None = None) -> int:
    ...
    checks.append(require(
        ...
        "selector experiment validation reads current_evidence_summary.json.rebuild_validation_contract before writing artifacts and keeps the rebuild order as provenance metadata only",
    ))
    checks.append(require(
        "## Current Evidence Boundary" in first_22
        and "historical selector-scoring research on frozen walk-forward artifacts" in first_22
        and "ROI dampening improved the selector replay" in first_22,
        "current_evidence_boundary_present",
        "selector experiment report opens with a current-evidence boundary before purpose/method details",
    ))
    checks.append(require(
    ...
    checks.append(require(
        ...
        "selector_experiment_valid_evidence_scope_visible",
        "selector experiment report, generator, and direct validator now expose the raw valid_evidence_scope=selector_scoring_replay_diagnostic_only marker so copied replay-diagnostic rows stay out of paper-trade, promotion, live-profitability, bankroll, and real-money evidence lanes",
    ))
    checks.append(require(
        "Valid use: compare ROI-dampening variants against the original +22.46% train-only selector" in first_22
        and "Do not treat the `Recommended action` line below as permission to change the current paper basket" in first_22
        and "override `forward_evidence_scorecard.txt`" in first_22,
        "valid_use_as_selector_research",
        "top boundary keeps the sqrt|strict recommendation in research context rather than current paper-basket authority",
    ))
    checks.append(require(
    ...
    checks.append(require(
        "fold-2017 improvement, BEL bridge rows, and CD selection details are replay diagnostics" in first_22
        and "already-mined candidate rules" in first_22
        and "not a fresh from-scratch discovery loop" in first_22
        and "not proof that CD_REFINED, CD_CORE, or a selector variant should displace OP anchor evidence" in first_22,
        "replay_diagnostic_limitation_visible",
        "top boundary labels fold, BEL-bridge, and CD details as replay diagnostics rather than fresh discovery or OP-anchor displacement proof",
    ))
    checks.append(require(
        rebuild_line in first_22
        and rebuild_line in generator_text,
        "current_bridge_rebuild_route_visible",
        "selector experiment report and generator publish the settlement-audit -> current-bridge -> bridge-validator route as provenance metadata only",
    ))
    checks.append(require(
        "Do not substitute `BAQ` for dormant `BEL`" in first_22
        and "validate this boundary with `python3 validate_selector_experiment_caution.py`" in first_22,
        "baq_bel_and_validator_route_visible",
        "top boundary keeps the BAQ/BEL guardrail and names the direct validator route",
    ))
    checks.append(require(
        "**Baseline (raw|strict):** +22.46% ROI" in text
        and "**Best (sqrt|strict):** +30.42% ROI" in text
        and "**Delta:** +7.96pp" in text,
        "selector_delta_visible",
        "report still shows the historical selector replay improvement over the raw baseline",
    ))
    checks.append(require(
        "The **sqrt|strict** variant improves the historical walk-forward replay by **+7.96pp**" in text
        and "**Recommended action:** keep `sqrt|strict` as the best tested selector-replay variant for research comparisons" in text
        and "do not change the current paper basket from this experiment alone" in text,
        "recommendation_still_present_but_bounded",
        "report preserves the historical selector recommendation while explicitly preventing over-reading it as a live posture change",
    ))
    checks.append(require(
        "The mechanism of improvement is not a clean CD_CORE-over-CD_REFINED fix" in text
        and "The largest replay win is fold 2017" in text
        and "Guardrail relaxation (`relaxed_n150`) has zero effect" in text
        and "CD_CORE_K8 is still selected in 1/10 folds" in text,
        "mechanism_caveat_still_visible",
        "report keeps the mechanism caveat visible beside the headline improvement and prevents the old CD-bottleneck overread",
    ))
    checks.append(require(
        "## Current Evidence Boundary" in generator_text
        and "Do not substitute `BAQ` for dormant `BEL`" in generator_text
        and "validate_selector_experiment_caution.py" in generator_text,
        "generator_preserves_boundary",
        "experiment_selector_variants.py will regenerate the same current-evidence boundary",
    ))

    current_read = (
        "SELECTOR_EXPERIMENT.md is validated as historical selector-scoring research: sqrt|strict improved the frozen replay "
        "from +22.46% to +30.42%, but the fold-2017 win, BEL bridge rows, and CD details are replay diagnostics on already-mined candidates, "
    ...
    payload: dict[str, Any] = {
        "suite": "selector_experiment_caution",
        "suite_status": "pass",
        ...
        "evidence_boundary": {
            "artifact_role": "selector-scoring experiment caution validator",
            "valid_evidence_scope": VALID_EVIDENCE_SCOPE,
    ...
    md_lines = [
        "# Selector Experiment Caution Validation",
        "",
